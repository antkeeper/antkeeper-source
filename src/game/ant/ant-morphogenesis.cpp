...
static geom::aabb<float> calculate_bounds(const std::byte* vertex_data, std::size_t index_count, const gl::vertex_attribute& position_attribute);
...
static std::unique_ptr<render::material> build_exoskeleton_material
(
...
std::unique_ptr<render::model> ant_morphogenesis(const ant_phenome& phenome)
{
	...
	const gl::vertex_buffer* forewings_vbo = (wings_present) ? forewings_model->get_vertex_buffer().get() : nullptr;
	...
	// Setup model VAO
	gl::vertex_array* model_vao = model->get_vertex_array().get();
	...
	std::uint32_t mesosoma_index_count = (mesosoma_model->get_groups()).front().index_count;
	std::uint32_t legs_index_count = (legs_model->get_groups()).front().index_count;
	std::uint32_t head_index_count = (head_model->get_groups()).front().index_count;
	std::uint32_t mandibles_index_count = (mandibles_model->get_groups()).front().index_count;
	std::uint32_t antennae_index_count = (antennae_model->get_groups()).front().index_count;
	std::uint32_t waist_index_count = (waist_model->get_groups()).front().index_count;
	std::uint32_t gaster_index_count = (gaster_model->get_groups()).front().index_count;
	std::uint32_t sting_index_count = (sting_present) ? (sting_model->get_groups()).front().index_count : 0;
	std::uint32_t eyes_index_count = (eyes_present) ? (eyes_model->get_groups()).front().index_count : 0;
	std::uint32_t lateral_ocelli_index_count = (lateral_ocelli_present) ? (lateral_ocelli_model->get_groups()).front().index_count : 0;
	std::uint32_t median_ocellus_index_count = (median_ocellus_present) ? (median_ocellus_model->get_groups()).front().index_count : 0;
	std::uint32_t forewings_index_count = (wings_present) ? (forewings_model->get_groups()).front().index_count : 0;
	std::uint32_t hindwings_index_count = (wings_present) ? (hindwings_model->get_groups()).front().index_count : 0;
	std::uint32_t exoskeleton_index_count =
	...
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_procoxa_l_indices, procoxa_l_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_profemur_l_indices;
	if (auto it = legs_skeleton.bone_map.find("profemur_l"); it != legs_skeleton.bone_map.end())
		old_profemur_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_profemur_l_indices, profemur_l_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_protibia_l_indices;
	if (auto it = legs_skeleton.bone_map.find("protibia_l"); it != legs_skeleton.bone_map.end())
		old_protibia_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_protibia_l_indices, protibia_l_bone_index, legs_to_body);
	...
	if (auto it = legs_skeleton.bone_map.find("protarsus5_l"); it != legs_skeleton.bone_map.end())
		old_protarsus_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_protarsus_l_indices, protarsus_l_bone_index, legs_to_body);
	
	std::unordered_set<std::uint8_t> old_procoxa_r_indices;
	if (auto it = legs_skeleton.bone_map.find("procoxa_r"); it != legs_skeleton.bone_map.end())
		old_procoxa_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_procoxa_r_indices, procoxa_r_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_profemur_r_indices;
	if (auto it = legs_skeleton.bone_map.find("profemur_r"); it != legs_skeleton.bone_map.end())
		old_profemur_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_profemur_r_indices, profemur_r_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_protibia_r_indices;
	if (auto it = legs_skeleton.bone_map.find("protibia_r"); it != legs_skeleton.bone_map.end())
		old_protibia_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_protibia_r_indices, protibia_r_bone_index, legs_to_body);
	...
	if (auto it = legs_skeleton.bone_map.find("protarsus5_r"); it != legs_skeleton.bone_map.end())
		old_protarsus_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_protarsus_r_indices, protarsus_r_bone_index, legs_to_body);
	
	std::unordered_set<std::uint8_t> old_mesocoxa_l_indices;
	if (auto it = legs_skeleton.bone_map.find("mesocoxa_l"); it != legs_skeleton.bone_map.end())
		old_mesocoxa_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesocoxa_l_indices, mesocoxa_l_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_mesofemur_l_indices;
	if (auto it = legs_skeleton.bone_map.find("mesofemur_l"); it != legs_skeleton.bone_map.end())
		old_mesofemur_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesofemur_l_indices, mesofemur_l_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_mesotibia_l_indices;
	if (auto it = legs_skeleton.bone_map.find("mesotibia_l"); it != legs_skeleton.bone_map.end())
		old_mesotibia_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesotibia_l_indices, mesotibia_l_bone_index, legs_to_body);
	...
	if (auto it = legs_skeleton.bone_map.find("mesotarsus5_l"); it != legs_skeleton.bone_map.end())
		old_mesotarsus_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesotarsus_l_indices, mesotarsus_l_bone_index, legs_to_body);
	
	std::unordered_set<std::uint8_t> old_mesocoxa_r_indices;
	if (auto it = legs_skeleton.bone_map.find("mesocoxa_r"); it != legs_skeleton.bone_map.end())
		old_mesocoxa_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesocoxa_r_indices, mesocoxa_r_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_mesofemur_r_indices;
	if (auto it = legs_skeleton.bone_map.find("mesofemur_r"); it != legs_skeleton.bone_map.end())
		old_mesofemur_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesofemur_r_indices, mesofemur_r_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_mesotibia_r_indices;
	if (auto it = legs_skeleton.bone_map.find("mesotibia_r"); it != legs_skeleton.bone_map.end())
		old_mesotibia_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesotibia_r_indices, mesotibia_r_bone_index, legs_to_body);
	...
	if (auto it = legs_skeleton.bone_map.find("mesotarsus5_r"); it != legs_skeleton.bone_map.end())
		old_mesotarsus_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_mesotarsus_r_indices, mesotarsus_r_bone_index, legs_to_body);
	
	std::unordered_set<std::uint8_t> old_metacoxa_l_indices;
	if (auto it = legs_skeleton.bone_map.find("metacoxa_l"); it != legs_skeleton.bone_map.end())
		old_metacoxa_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metacoxa_l_indices, metacoxa_l_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_metafemur_l_indices;
	if (auto it = legs_skeleton.bone_map.find("metafemur_l"); it != legs_skeleton.bone_map.end())
		old_metafemur_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metafemur_l_indices, metafemur_l_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_metatibia_l_indices;
	if (auto it = legs_skeleton.bone_map.find("metatibia_l"); it != legs_skeleton.bone_map.end())
		old_metatibia_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metatibia_l_indices, metatibia_l_bone_index, legs_to_body);
	...
	if (auto it = legs_skeleton.bone_map.find("metatarsus5_l"); it != legs_skeleton.bone_map.end())
		old_metatarsus_l_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metatarsus_l_indices, metatarsus_l_bone_index, legs_to_body);
	
	std::unordered_set<std::uint8_t> old_metacoxa_r_indices;
	if (auto it = legs_skeleton.bone_map.find("metacoxa_r"); it != legs_skeleton.bone_map.end())
		old_metacoxa_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metacoxa_r_indices, metacoxa_r_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_metafemur_r_indices;
	if (auto it = legs_skeleton.bone_map.find("metafemur_r"); it != legs_skeleton.bone_map.end())
		old_metafemur_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metafemur_r_indices, metafemur_r_bone_index, legs_to_body);
	std::unordered_set<std::uint8_t> old_metatibia_r_indices;
	if (auto it = legs_skeleton.bone_map.find("metatibia_r"); it != legs_skeleton.bone_map.end())
		old_metatibia_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metatibia_r_indices, metatibia_r_bone_index, legs_to_body);
	...
	if (auto it = legs_skeleton.bone_map.find("metatarsus5_r"); it != legs_skeleton.bone_map.end())
		old_metatarsus_r_indices.emplace(it->second);
	reskin_vertices(vertex_buffer_data.data() + legs_vbo_offset, legs_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_metatarsus_r_indices, metatarsus_r_bone_index, legs_to_body);
	...
	reskin_vertices(vertex_buffer_data.data() + head_vbo_offset, head_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_head_bone_indices, head_bone_index, head_to_body);
	...
	reskin_vertices(vertex_buffer_data.data() + mandibles_vbo_offset, mandibles_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_head_mandible_l_bone_indices, mandible_l_bone_index, mandible_l_to_body);
	...
	reskin_vertices(vertex_buffer_data.data() + mandibles_vbo_offset, mandibles_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_head_mandible_r_bone_indices, mandible_r_bone_index, mandible_r_to_body);
	...
	reskin_vertices(vertex_buffer_data.data() + antennae_vbo_offset, antennae_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_antennomere1_l_indices, antennomere1_l_bone_index, antenna_l_to_body);
	...
	reskin_vertices(vertex_buffer_data.data() + waist_vbo_offset, waist_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_petiole_bone_indices, petiole_bone_index, waist_to_body);
	if (postpetiole_present)
	{
		...
		reskin_vertices(vertex_buffer_data.data() + waist_vbo_offset, waist_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_postpetiole_bone_indices, postpetiole_bone_index, waist_to_body);
	}
	...
	reskin_vertices(vertex_buffer_data.data() + gaster_vbo_offset, gaster_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_gaster_bone_indices, gaster_bone_index, gaster_to_body);
	...
	if (sting_present)
	{
		...
		reskin_vertices(vertex_buffer_data.data() + sting_vbo_offset, sting_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_sting_bone_indices, sting_bone_index, sting_to_body);
	}
	...
	if (eyes_present)
	{
		...
		reskin_vertices(vertex_buffer_data.data() + eyes_vbo_offset, eyes_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_eye_l_bone_indices, head_bone_index, eye_l_to_body);
		...
		reskin_vertices(vertex_buffer_data.data() + eyes_vbo_offset, eyes_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_eye_r_bone_indices, head_bone_index, eye_r_to_body);
	}
	...
	if (lateral_ocelli_present)
	{
		...
		reskin_vertices(vertex_buffer_data.data() + lateral_ocelli_vbo_offset, lateral_ocelli_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_ocellus_l_bone_indices, head_bone_index, ocellus_l_to_body);
		...
		reskin_vertices(vertex_buffer_data.data() + lateral_ocelli_vbo_offset, lateral_ocelli_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_ocellus_r_bone_indices, head_bone_index, ocellus_r_to_body);
	}
	...
	if (median_ocellus_present)
	{
		...
		reskin_vertices(vertex_buffer_data.data() + median_ocellus_vbo_offset, median_ocellus_index_count, *position_attribute, *normal_attribute, *tangent_attribute, *bone_index_attribute, old_ocellus_m_bone_indices, head_bone_index, ocellus_m_to_body);
	}
	...
	render::model_group& exoskeleton_group = model->get_groups().back();
	...
	exoskeleton_group.start_index = 0;
	exoskeleton_group.index_count = exoskeleton_index_count;
	
	std::uint32_t index_offset = exoskeleton_index_count;
	if (eyes_present)
	{
		// Construct eyes model group
		model->get_groups().resize(model->get_groups().size() + 1);
		render::model_group& eyes_group = model->get_groups().back();
		eyes_group.id = "eyes";
		...
		eyes_group.index_count = eyes_index_count;
	...
	if (lateral_ocelli_present || median_ocellus_present)
	{
		...
		model->get_groups().resize(model->get_groups().size() + 1);
		render::model_group& ocelli_group = model->get_groups().back();
		...
		ocelli_group.index_count = index_count;
	}
	...
	if (wings_present)
	{
		...
		model->get_groups().resize(model->get_groups().size() + 1);
		render::model_group& forewings_group = model->get_groups().back();
		...
		forewings_group.index_count = forewings_index_count;
		index_offset += forewings_index_count;
		
		// Construct hindwings model group
		model->get_groups().resize(model->get_groups().size() + 1);
		render::model_group& hindwings_group = model->get_groups().back();
		...
		hindwings_group.index_count = hindwings_index_count;
...
geom::aabb<float> calculate_bounds(const std::byte* vertex_data, std::size_t index_count, const gl::vertex_attribute& position_attribute)
{
	...
	{
		// Get vertex position
		const float* x = reinterpret_cast<const float*>(position_data + position_attribute.stride * i);
		const float* y = x + 1;
		const float* z = y + 1;
