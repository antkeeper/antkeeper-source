...
namespace engine::physics
{
	...
	std::optional<std::tuple<float, u32, math::fvec3>> mesh_collider::intersection(const geom::ray<float, 3>& ray) const
	{
		if (!m_mesh)
		{
			return std::nullopt;
		...
		float nearest_face_distance = math::inf<float>;
		u32 nearest_face_index;
