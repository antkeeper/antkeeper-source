...
namespace engine::render
{
	...
	void material::set_flags(u32 flags) noexcept
	{
		m_flags = flags;
	...
	void material::rehash() noexcept
	{
		...
		m_hash = hash::combine_hash(m_hash, std::hash<u32>{}(m_flags));
	}
}
...
namespace engine::resources
{
	...
	template <>
	std::unique_ptr<render::material> resource_loader<render::material>::load(resources::resource_manager& resource_manager, std::shared_ptr<deserialize_context> ctx)
	{
		...
		if (depth_mode == "in_front")
			flags |= render::material_flag::x_ray;

		// Read decal mode
		std::string decal_mode;
		read_value(&decal_mode, *json, "decal_mode");
		if (decal_mode == "decal")
			flags |= render::material_flag::decal;
		else if (decal_mode == "surface")
			flags |= render::material_flag::decal_surface;
		...
		if (auto variables_element = json->find("variables"); variables_element != json->end())
		{
			for (const auto& variable_element : variables_element.value())
			{
				...
				{
					usize columns = std::stoul(type.substr(type.size() - 3, 1));
					usize rows = std::stoul(type.substr(type.size() - 1, 1));
				...
				{
					usize size = std::stoul(type.substr(type.size() - 1, 1));
