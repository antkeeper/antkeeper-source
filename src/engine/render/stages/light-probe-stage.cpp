...
namespace engine::render
{
	...
	light_probe_stage::light_probe_stage(gl::pipeline& pipeline, resources::resource_manager& resource_manager):
		m_pipeline(&pipeline)
	{
		// Generate restricted mip range samplers
		m_downsample_samplers.resize(16);
		m_filter_samplers.resize(16);
		for (usize i = 0; i < m_downsample_samplers.size(); ++i)
		{
		...
		(
			...
			(
				...
				(
					...
					static_cast<u32>(m_cubemap_filter_sample_count),
					static_cast<u32>(m_cubemap_filter_mip_count - 1)
				)
			),
	...
	void light_probe_stage::update_light_probes_luminance(const std::vector<scene::object_base*>& light_probes)
	{
		...
		for (const auto& object: light_probes)
		{
			...
			for (usize i = 1; i < light_probe.get_luminance_framebuffers().size(); ++i)
			{
	...
	void light_probe_stage::set_sh_sample_count(usize count)
	{
	...
	void light_probe_stage::set_cubemap_filter_sample_count(usize count)
	{
	...
	void light_probe_stage::rebuild_cubemap_to_sh_shader_program()
	{
		...
		{
			log_error("Failed to build cubemap to spherical harmonics shader program: {}", m_cubemap_to_sh_shader_program->info());
			log_warning("{}", m_cubemap_to_sh_shader_template->configure(gl::shader_stage::vertex));
	...
	void light_probe_stage::rebuild_cubemap_downsample_shader_program()
	{
		...
		{
			log_error("Failed to build cubemap downsample shader program: {}", m_cubemap_downsample_shader_program->info());
			log_warning("{}", m_cubemap_downsample_shader_template->configure(gl::shader_stage::vertex));
	...
	void light_probe_stage::rebuild_cubemap_filter_lut_shader_program()
	{
		...
		{
			log_error("Failed to build cubemap filter LUT shader program: {}", m_cubemap_filter_lut_shader_program->info());
			log_warning("{}", m_cubemap_filter_lut_shader_template->configure(gl::shader_stage::vertex));
	...
	void light_probe_stage::rebuild_cubemap_filter_shader_program()
	{
		...
		{
			log_error("Failed to build cubemap filter shader program: {}", m_cubemap_filter_shader_program->info());
			log_warning("{}", m_cubemap_filter_shader_template->configure(gl::shader_stage::vertex));
