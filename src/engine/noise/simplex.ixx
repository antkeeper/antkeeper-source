...
export module engine.noise.simplex;
...
export namespace engine::noise
{
	...
	/// @private
	template <usize N>
	inline constexpr usize simplex_corner_count = usize{2} << math::max(usize{0}, N - 1);

	/// Number of edges in an *n*-dimensional simplex lattice cell.
	/// @private
	template <usize N>
	inline constexpr usize simplex_edge_count = (N > 1) ? N * simplex_corner_count<N - 1> : 2;

	/// Returns the simplex lattice cell corner vector for a given dimension and index.
	/// @private
	template <class T, usize N, usize... I>
	[[nodiscard]] constexpr math::vector<T, N> make_simplex_corner(usize i, std::index_sequence<I...>)
	{
	...
	inline constexpr auto simplex_corners = make_simplex_corners<T, N>(std::make_index_sequence<simplex_corner_count<N>>{});

	/// Returns the simplex lattice cell edge vector for a given dimension and index.
	/// @private
	template <class T, usize N, usize... I>
	[[nodiscard]] constexpr math::vector<T, N> make_simplex_edge(usize i, std::index_sequence<I...>)
	{
		usize j = i / (simplex_edge_count<N> / N);
	...
	/// @private
	template <class T, usize N>
	inline constexpr auto simplex_edges = make_simplex_edges<T, N>(std::make_index_sequence<simplex_edge_count<N>>{});
	...
	)
	{
		...
		math::vector<usize, N> axis_order;
		for (usize i = 0; i < N; ++i)
			axis_order[i] = i;
		std::sort
		(
