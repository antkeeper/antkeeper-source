...
#include <engine/resources/text-file.hpp>
	...
	for (std::size_t i = 0; i < source.size(); ++i)
	{
			...
			line_stream >> token && token == "include")
		{
				...
				(token.front() == '<' && token.back() == '>')))
			{
				...
				// Load include file
				const text_file* include_file = resource_manager->load<text_file>(path);
				if (!include_file)
				{
...
gl::shader_program* resource_loader<gl::shader_program>::load(resource_manager* resource_manager, PHYSFS_File* file, const std::filesystem::path& path)
{
	...
	const text_file* source_file = resource_loader<text_file>::load(resource_manager, file, path);
