...
export module engine.resources.resource_manager;
...
export namespace engine::resources
{
	...
	std::shared_ptr<T> resource_manager::load(const std::filesystem::path& path)
	{
		...
		try
		{
			log_debug("Loading resource \"{}\"...", path_string);

			// Load and cache resource
			std::shared_ptr<T> resource = resource_loader<T>::load(*this, open_read(path));
			resource_cache[path] = resource;

			log_debug("Loading resource \"{}\"... OK", path_string);

			return resource;
		}
		catch (const std::exception& e)
		{
			log_error("Failed to load resource \"{}\": {}", path_string, e.what());
			log_debug("Loading resource \"{}\"... FAILED", path_string);
		}
	...
	bool resource_manager::save(const T& resource, const std::filesystem::path& path) const
	{
		...
		try
		{
			log_debug("Saving resource to \"{}\"...", path_string);
			...
			log_debug("Saving resource to \"{}\"... OK", path_string);

			return true;
		}
		catch (const std::exception& e)
		{
			log_error("Failed to save resource to \"{}\": {}", path_string, e.what());
			log_debug("Saving resource to \"{}\"... FAILED", path_string);
		}
