...
namespace std
{
	...
	template <>
	struct hash<engine::hash::fnv32_t>
	{
		[[nodiscard]] inline constexpr engine::usize operator()(const engine::hash::fnv32_t& h) const noexcept
		{
			return static_cast<engine::usize>(h);
		}
	};
	
	/// Hash specialization for 64-bit FNV hash values.
	template <>
	struct hash<engine::hash::fnv64_t>
	{
		[[nodiscard]] inline constexpr engine::usize operator()(const engine::hash::fnv64_t& h) const noexcept
		{
			return static_cast<engine::usize>(h);
		}
	};
