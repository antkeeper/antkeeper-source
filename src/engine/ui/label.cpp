...
void label::reposition_text()
{
	...
	m_text_object->set_translation({std::round(translation.x()), std::round(translation.y()), m_text_object->get_translation().z()});
}
...
void label::recalculate_min_size()
{
	if (auto font = m_text_object->get_font())
	{
		...
		set_min_size({m_text_object->get_bounds().size().x(), std::round(metrics.em_size)});

		// With internal leading
		//set_min_size({m_text_object->get_bounds().size().x(), std::round(metrics.ascent - metrics.descent)});
	}
	else
	{
