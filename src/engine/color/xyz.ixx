...
export module engine.color.xyz;
...
export namespace engine::color
{
	...
	template <class T>
	[[nodiscard]] inline constexpr T xyz_to_luminance(const vec3<T>& x) noexcept
	{
		return x[1];
	}
	...
	template <class T>
	[[nodiscard]] constexpr vec3<T> xyz_to_xyy(const vec3<T>& x) noexcept
	{
		const T sum = x[0] + x[1] + x[2];
		return vec3<T>{x[0] / sum, x[1] / sum, x[1]};
	}
	...
	template <class T>
	[[nodiscard]] vec3<T> xyz_match(T lambda)
	{
		return vec3<T>
		{
