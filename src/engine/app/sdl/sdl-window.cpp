...
namespace engine::app
{
		...
		window(window_manager)
	{
		...
		if (!m_internal_window)
		{
			...
			log_debug("Creating SDL window... FAILED");
			throw std::runtime_error(std::move(error_message));
		}

		log_debug("Creating SDL window... OK");
		...
		if (!m_internal_context)
		{
			...
			log_fatal("{}", error_message);
			log_debug("Creating OpenGL context... FAILED");
			throw std::runtime_error(std::move(error_message));
		}
		log_debug("Creating OpenGL context... OK");
		...
		{
			log_warning("Requested OpenGL context version {}.{} but got version {}.{}", config::opengl_version_major, config::opengl_version_minor, opengl_context_version_major, opengl_context_version_minor);
		}
		...
		if (!gladLoadGL(reinterpret_cast<GLADloadfunc>(SDL_GL_GetProcAddress)))
		{
			...
			log_fatal("{}", error_message);
			log_debug("Loading OpenGL functions... FAILED");
			throw std::runtime_error(std::move(error_message));
		}
		log_debug("Loading OpenGL functions... OK");
	...
	void sdl_window::set_v_sync(bool v_sync)
	{
		if (v_sync)
		{
			log_debug("Enabling adaptive v-sync...");
			if (!SDL_GL_SetSwapInterval(-1))
			{
				log_error("Failed to enable adaptive v-sync: {}", SDL_GetError());
				log_debug("Enabling adaptive v-sync... FAILED");
				log_debug("Enabling synchronized v-sync...");
				if (!SDL_GL_SetSwapInterval(1))
				{
					log_error("Failed to enable synchronized v-sync: {}", SDL_GetError());
					log_debug("Enabling synchronized v-sync... FAILED");
					v_sync = false;
				}
				else
				{
					log_debug("Enabling synchronized v-sync... OK");
				}
			}
			else
			{
				log_debug("Enabling adaptive v-sync... OK");
			}
		}
		else
		{
			log_debug("Disabling v-sync...");
			if (!SDL_GL_SetSwapInterval(0))
			{
				log_error("Failed to disable v-sync: {}", SDL_GetError());
				log_debug("Disabling v-sync... FAILED");
				v_sync = true;
			}
			else
			{
				log_debug("Disabling v-sync... OK");
			}
		}
	...
	std::shared_ptr<display> sdl_window::get_display() const
	{
		...
		if (!sdl_display_id)
		{
			log_error("Failed to get SDL display for window: {}", SDL_GetError());
			SDL_ClearError();
			return nullptr;
		}
		...
		{
			log_error("SDL display unrecognized by SDL window manager.");
			return nullptr;
		}
