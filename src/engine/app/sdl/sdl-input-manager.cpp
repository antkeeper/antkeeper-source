...
namespace engine::app
{
	sdl_input_manager::sdl_input_manager()
	{
		...
		log_debug("Initializing SDL joystick and controller subsystems...");
		if (!SDL_InitSubSystem(SDL_INIT_JOYSTICK | SDL_INIT_GAMEPAD))
		{
			...
			log_debug("Initializing SDL joystick and controller subsystems... FAILED");
			throw std::runtime_error(std::move(error_message));
		}
		else
		{
			log_debug("Initializing SDL joystick and controller subsystems... OK");
		}
	...
	sdl_input_manager::~sdl_input_manager()
	{
		...
		log_debug("Quitting SDL joystick and controller subsystems...");
		SDL_QuitSubSystem(SDL_INIT_JOYSTICK | SDL_INIT_GAMEPAD);
		log_debug("Quitting SDL joystick and controller subsystems... OK");
	}

	void sdl_input_manager::update()
	{
		// Active modifier keys
		u16 sdl_key_mod = SDL_KMOD_NONE;
		u16 modifier_keys = input::modifier_key::none;
		...
		for (;;)
		{
			...
			{
				case SDL_EVENT_QUIT:
					log_debug("Received application quit request");
		...
		for (;;)
		{
			...
			switch (event.type)
			{
				...
				{
					m_keyboard.edit_text(event.edit.text, static_cast<usize>(event.edit.start), static_cast<usize>(event.edit.length));
					break;
				}
				...
				[[unlikely]] case SDL_EVENT_GAMEPAD_ADDED:
				{
					if (SDL_IsGamepad(event.gdevice.which))
					{
						...
						if (sdl_controller)
						{
							...
							{
								// Gamepad reconnected
								log_info("Reconnected gamepad {}", event.gdevice.which);
								it->second->connect();
							}
							else
							{
								...
								log_info("Connected gamepad {}; name: \"{}\"; UUID: {}", event.gdevice.which, controller_name, gamepad_uuid.string());
						...
						{
							log_error("Failed to connect gamepad {}: {}", event.gdevice.which, SDL_GetError());
							SDL_ClearError();
						}
					}
				...
				[[unlikely]] case SDL_EVENT_GAMEPAD_REMOVED:
				{
					...
					if (sdl_controller)
					{
						...
						log_info("Disconnected gamepad {}", event.gdevice.which);
					}
	...
	void sdl_input_manager::set_cursor_visible(bool visible)
	{
		...
		{
			log_error("Failed to set cursor visibility: \"{}\"", SDL_GetError());
			SDL_ClearError();
		}
	}
	...
	void sdl_input_manager::set_clipboard_text(const std::string& text)
	{
		...
		{
			log_error("Failed to set clipboard text: \"{}\"", SDL_GetError());
			SDL_ClearError();
		}
	}
