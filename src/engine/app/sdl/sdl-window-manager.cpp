...
namespace engine::app
{
	sdl_window_manager::sdl_window_manager()
	{
		...
		log_debug("Initializing SDL events and video subsystems...");
		if (!SDL_InitSubSystem(SDL_INIT_EVENTS | SDL_INIT_VIDEO))
		{
			...
			log_debug("Initializing SDL events and video subsystems... FAILED");
			throw std::runtime_error(std::move(error_message));
		}
		log_debug("Initializing SDL events and video subsystems... OK");
		...
		if (!sdl_display_ids)
		{
			log_error("Failed to get SDL displays: {}", SDL_GetError());
			SDL_ClearError();
		}
		else
		{
			...
			{
				log_warning("No displays detected.");
			}
			else
			{
				log_info("Display count: {}", sdl_display_count);
				...
				for (int i = 0; i < sdl_display_count; ++i)
				{
					...
					log_info("Display {} name: \"{}\"; resolution: {}x{}; refresh rate: {}Hz", i, display.get_name(), display_resolution.x(), display_resolution.y(), display.get_refresh_rate());
				}
			}
		...
		// Load OpenGL library
		log_debug("Loading OpenGL library...");
		if (!SDL_GL_LoadLibrary(nullptr))
		{
			auto error_message = std::format("Failed to load OpenGL library: {}", SDL_GetError());
			log_fatal("{}", error_message);
			log_debug("Loading OpenGL library... FAILED");
			throw std::runtime_error(std::move(error_message));
		}
		log_debug("Loading OpenGL library... OK");
	...
	sdl_window_manager::~sdl_window_manager()
	{
		...
		log_debug("Quitting SDL video subsystem...");
		SDL_QuitSubSystem(SDL_INIT_VIDEO);
		log_debug("Quitting SDL video subsystem... OK");
	}
		...
		for (;;)
		{
			...
			switch (event.type)
			{
				case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
				{
					...
					(
						static_cast<u32>(window->m_viewport_size.x()),
						static_cast<u32>(window->m_viewport_size.y())
					);

					// Log window resized event
					log_debug("Window {} resized to {}x{}", event.window.windowID, event.window.data1, event.window.data2);
				...
				case SDL_EVENT_WINDOW_MOVED:
				{
					...
					log_debug("Window {} moved to ({}, {}).", event.window.windowID, event.window.data1, event.window.data2);
				...
				case SDL_EVENT_WINDOW_FOCUS_GAINED:
				{
					...
					log_debug("Window {} gained focus.", event.window.windowID);
				...
				case SDL_EVENT_WINDOW_FOCUS_LOST:
				{
					...
					log_debug("Window {} lost focus.", event.window.windowID);
				...
				case SDL_EVENT_WINDOW_MAXIMIZED:
				{
					...
					// Log window focus gained event
					log_debug("Window {} maximized.", event.window.windowID);
				...
				case SDL_EVENT_WINDOW_RESTORED:
				{
					...
					log_debug("Window {} restored.", event.window.windowID);
				...
				case SDL_EVENT_WINDOW_MINIMIZED:
				{
					...
					// Log window focus gained event
					log_debug("Window {} minimized.", event.window.windowID);
				...
				[[unlikely]] case SDL_EVENT_WINDOW_CLOSE_REQUESTED:
				{
					...
					log_debug("Window {} closed.", event.window.windowID);
				...
				[[unlikely]] case SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED:
				{
					...
					// Log window closed event
					log_debug("Window {} display scale changed to {}.", event.window.windowID, window->m_display_scale);
				...
				case SDL_EVENT_DISPLAY_ADDED:
					if (auto it = m_display_map.find(event.display.displayID); it != m_display_map.end())
					{
						...
						log_info("Reconnected display {}", it->second);
					...
					else
					{
						...
						// Log display info
						const auto display_resolution = display.get_bounds().size();
						log_info("Connected display {}; name: {}; resolution: {}x{}; refresh rate: {}Hz", it->second, display.get_name(), display_resolution.x(), display_resolution.y(), display.get_refresh_rate());
				...
				case SDL_EVENT_DISPLAY_REMOVED:
					if (auto it = m_display_map.find(event.display.displayID); it != m_display_map.end())
					{
						...
						log_info("Disconnected display {}.", it->second);
					...
					{
						log_error("SDL index of disconnected display ({}) out of range.", event.display.displayID);
					}
					break;
				...
				case SDL_EVENT_DISPLAY_ORIENTATION:
					if (auto it = m_display_map.find(event.display.displayID); it != m_display_map.end())
					{
						...
						log_info("Display {} orientation changed.", it->second);
					...
					{
						log_error("SDL index of orientation-changed display ({}) out of range.", event.display.displayID);
					}
					break;
	...
	std::shared_ptr<display> sdl_window_manager::get_primary_display() const
	{
		...
		{
			log_error("Failed to get primary display: {}", SDL_GetError());
			SDL_ClearError();
			return nullptr;
		}
		...
		{
			log_error("SDL primary display unrecognized by SDL window manager.");
			return nullptr;
		}
	...
	void sdl_window_manager::update_display(display& display, unsigned int sdl_display_index)
	{
		...
		{
			log_warning("Failed to get mode of display {}: {}", sdl_display_index, SDL_GetError());
			SDL_ClearError();
			sdl_display_mode = {};
		}
		else
		...
		if (!sdl_display_name)
		{
			log_warning("Failed to get name of display {}: {}", sdl_display_index, SDL_GetError());
			SDL_ClearError();
		...
		{
			log_warning("Failed to get bounds of display {}: {}", sdl_display_index, SDL_GetError());
			SDL_ClearError();
		...
		{
			log_warning("Failed to get usable bounds of display {}: {}", sdl_display_index, SDL_GetError());
			SDL_ClearError();
