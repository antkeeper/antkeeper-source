...
namespace engine::input
{
	...
	void gamepad::handle_axial_motion(gamepad_axis axis)
	{
		...
		if (abs(axis_position) > activation_min)
		{
			// Remap position according to activation thresholds and clamp to `[0, 1]`.
			float response = map_range(abs(axis_position), activation_min, activation_max, 0.0f, 1.0f);
	...
	void gamepad::handle_biaxial_motion(gamepad_axis axis_x, gamepad_axis axis_y)
	{
		...
		const float radius = min(x_activation_min, y_activation_min) * deadzone_roundness;
		const float dx = max(0.0f, abs(x_axis_position) - x_activation_min + radius);
		const float dy = max(0.0f, abs(y_axis_position) - y_activation_min + radius);
		const float distance = sqrt(dx * dx + dy * dy) - radius;
	
		if (distance > 0.0f)
		{
			const float nx = abs(x_axis_position) / distance;
			const float ny = abs(y_axis_position) / distance;
			...
			float response_x = clamp(nx * ndx, 0.0f, 1.0f);
			float response_y = clamp(ny * ndy, 0.0f, 1.0f);
