...
namespace engine::input
{
	...
	{
		if (math::abs(event.position) > 0.5f)
		{
			gamepad_axis_mapped_publisher.publish({gamepad_axis_mapping(event.gamepad, event.axis, math::signbit(event.position))});
		}
	}
	...
	void mapper::handle_mouse_moved(const mouse_moved_event& event)
	{
		...
		{
			mouse_motion_mapped_publisher.publish({mouse_motion_mapping(event.mouse, mouse_motion_axis::x, math::signbit(static_cast<float>(event.difference.x())))});
		}
	
		if (event.difference.y())
		{
			mouse_motion_mapped_publisher.publish({mouse_motion_mapping(event.mouse, mouse_motion_axis::y, math::signbit(static_cast<float>(event.difference.y())))});
		}
	}
	...
	void mapper::handle_mouse_scrolled(const mouse_scrolled_event& event)
	{
		if (event.velocity.x())
		{
			mouse_scroll_mapped_publisher.publish({mouse_scroll_mapping(event.mouse, mouse_scroll_axis::x, math::signbit(event.velocity.x()))});
		}
	
		if (event.velocity.y())
		{
			mouse_scroll_mapped_publisher.publish({mouse_scroll_mapping(event.mouse, mouse_scroll_axis::y, math::signbit(event.velocity.y()))});
		}
	}
}
