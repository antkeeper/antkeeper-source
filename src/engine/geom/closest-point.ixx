...
export module engine.geom.closest_point;
...
export namespace engine::geom
{
	...
	{
		return a.extrapolate(max(T{0}, dot(b - a.origin, a.direction)));
	}
	...
	[[nodiscard]] constexpr point<T, N> closest_point(const line_segment<T, N>& ab, const point<T, N>& c) noexcept
	{
		...
		const auto distance_ab = dot(c - ab.a, direction_ab);
		if (distance_ab <= T{0})
		{
			return ab.a;
		}
	...
	[[nodiscard]] constexpr std::tuple<point<T, N>, point<T, N>> closest_point(const line_segment<T, N>& ab, const line_segment<T, N>& cd) noexcept
	{
		...
		const auto sqr_length_ab = sqr_length(direction_ab);
		const auto sqr_length_cd = sqr_length(direction_cd);
		const auto cd_dot_ca = dot(direction_cd, direction_ca);
		...
		if (sqr_length_ab <= T{0})
		{
			...
			else
			{
				...
				{
					ab.a,
					cd.a + direction_cd * clamp(cd_dot_ca / sqr_length_cd, T{0}, T{1})
				};
			}
		}
		else
		{
			const auto ab_dot_ca = dot(direction_ab, direction_ca);
			...
			if (sqr_length_cd <= T{0})
			{
				...
				return
				{
					ab.a + direction_ab * clamp(-ab_dot_ca / sqr_length_ab, T{0}, T{1}),
					cd.a
				};
			}
			else
			{
				const auto ab_dot_cd = dot(direction_ab, direction_cd);

				const auto den = sqr_length_ab * sqr_length_cd - ab_dot_cd * ab_dot_cd;

				auto distance_ab = (den) ? clamp((ab_dot_cd * cd_dot_ca - ab_dot_ca * sqr_length_cd) / den, T{0}, T{1}) : T{0};
				...
				else if (distance_cd > T{1})
				{
					return
					{
						ab.a + direction_ab * clamp((ab_dot_cd - ab_dot_ca) / sqr_length_ab, T{0}, T{1}),
						cd.b
					};
				}
	...
	{
		return b - a.normal * (dot(a.normal, b) + a.constant);
	}
	...
	[[nodiscard]] constexpr std::tuple<point<T, 3>, triangle_region> closest_point(const point<T, 3>& a, const point<T, 3>& b, const point<T, 3>& c, const point<T, 3>& p) noexcept
	{
		...
		const auto ac = c - a;
		const auto ap = p - a;
		const auto ap_dot_ab = dot(ap, ab);
		const auto ap_dot_ac = dot(ap, ac);
		...
		const auto bc = c - b;
		const auto bp = p - b;
		const auto bp_dot_ba = dot(bp, a - b);
		const auto bp_dot_bc = dot(bp, bc);
		...
		const auto cp = p - c;
		const auto cp_dot_ca = dot(cp, a - c);
		const auto cp_dot_cb = dot(cp, b - c);
		...
		const auto pa = a - p;
		const auto pb = b - p;
		const auto vc = dot(n, cross(pa, pb));
	...
	[[nodiscard]] point<T, N> closest_point(const hypersphere<T, N>& a, const point<T, N>& b)
	{
		...
		const auto d = sqr_length(ab);
		return d > sqr(a.radius) ? a.center + ab * (a.radius / sqrt(d)) : b;
	}
	...
	[[nodiscard]] point<T, N> closest_point(const hypercapsule<T, N>& a, const point<T, N>& b)
	{
		...
		const auto cb = b - c;
		const auto d = sqr_length(cb);
		return d > sqr(a.radius) ? c + cb * (a.radius / sqrt(d)) : b;
	}
