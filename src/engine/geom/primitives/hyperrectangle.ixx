...
export module engine.geom.primitives.hyperrectangle;
...
export namespace engine::geom::inline primitives
{
	...
	struct hyperrectangle
	{
		...
		[[nodiscard]] T distance(const vector_type& point) const noexcept
		{
			...
			return math::length(math::max(math::zero<vector_type>, d)) + math::min<T>(T{0}, math::max(d));
		}
		...
		[[nodiscard]] constexpr vector_type corner(usize index) const noexcept
		{
			vector_type p;
