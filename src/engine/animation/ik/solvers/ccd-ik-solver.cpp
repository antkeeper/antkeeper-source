...
{
	ccd_ik_solver::ccd_ik_solver(ik_rig& ik_rig, usize root_bone_index, usize effector_bone_index):
		m_ik_rig{&ik_rig}
	{
	...
	void ccd_ik_solver::solve()
	{
		...
		for (usize i = 0; i < m_max_iterations; ++i)
		{
			for (usize j = 0; j < m_bone_indices.size(); ++j)
			{
