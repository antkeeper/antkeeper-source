...
namespace engine::animation
{
	...
	float animation_sequence::duration() const
	{
		...
		{
			max_duration = math::max(max_duration, track.duration());
		}
...
namespace engine::resources
{
	...
	void deserializer<animation_sequence>::deserialize(animation_sequence& sequence, deserialize_context& ctx)
	{
		...
		for (const auto& [track_path, track_element] : json.at("tracks").items())
		{
			...
			// Load channels
			for (usize i = 0; i < track.channels().size(); ++i)
			{
				...
				for (usize j = 0; j < keyframes_element.size(); j += 2)
				{
	...
	template <>
	std::unique_ptr<animation_sequence> resource_loader<animation_sequence>::load(resource_manager&, std::shared_ptr<deserialize_context> ctx)
	{
