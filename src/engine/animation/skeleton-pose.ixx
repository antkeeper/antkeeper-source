...
export module engine.animation.skeleton_pose;
...
export namespace engine::animation
{
	...
	public:
		...
		explicit skeleton_pose(animation::skeleton& skeleton);
		...
		void set_relative_transform(usize index, const math::transform<float>& transform);

		/// Sets the relative translation of a bone pose.
		/// @param index Index of a bone.
		/// @param translation Relative translation of the bone pose.
		void set_relative_translation(usize index, const math::transform<float>::vector_type& translation);

		/// Sets the relative rotation of a bone pose.
		/// @param index Index of a bone.
		/// @param rotation Relative rotation of the bone pose.
		void set_relative_rotation(usize index, const math::transform<float>::quaternion_type& rotation);

		/// Sets the relative scale of a bone pose.
		/// @param index Index of a bone.
		/// @param scale Relative scale of the bone pose.
		void set_relative_scale(usize index, const math::transform<float>::vector_type& scale);

		/// Sets the absolute transform describing a bone pose.
		/// @param index Index of a bone.
		/// @param transform Absolute transform describing the bone pose.
		void set_absolute_transform(usize index, const math::transform<float>& transform);

		/// Returns the relative transform describing a bone pose.
		/// @param index Index of a bone.
		/// @return Relative transform describing the bone pose.
		[[nodiscard]] inline const math::transform<float>& get_relative_transform(usize index) const
		{
		...
		[[nodiscard]] const math::transform<float>& get_absolute_transform(usize index) const;
		...
		[[nodiscard]] const math::fmat4& get_skinning_matrix(usize index) const;
	...
	protected:
		...
		[[nodiscard]] bool is_absolute_transform_outdated(usize index) const noexcept;
		...
		[[nodiscard]] bool is_inverse_absolute_transform_outdated(usize index) const noexcept;
		...
		[[nodiscard]] bool is_skinning_matrix_outdated(usize index) const noexcept;

		/// Updates the absolute transform of a bone and all of its outdated ancestors.
		/// @param index Index of a bone.
		void update_absolute_transform(usize index) const;
		...
		virtual void update_skinning_matrix(usize index) const;
		...
		mutable std::vector<u8> m_bone_flags;
