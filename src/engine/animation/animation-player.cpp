		...
		if (m_position >= m_sequence_duration)
		{
			if (m_looping)
			{
				if (m_sequence_duration > 0.0f)
				{
					...
					const auto looped_position = math::mod(m_position, m_sequence_duration);

					// Calculate number of times looped
					loop_count = static_cast<usize>(m_position / m_sequence_duration);
