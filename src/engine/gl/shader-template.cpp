...
namespace engine::gl
{
	...
	std::unique_ptr<gl::shader_program> shader_template::build(const dictionary_type& definitions) const
	{
		...
		if (has_vertex_directive())
		{
			...
			{
				log_error("Failed to build shader program from shader template: Failed to compile vertex shader object.");
				return nullptr;
			}
		}
		...
		if (has_fragment_directive())
		{
			...
			{
				log_error("Failed to build shader program from shader template: Failed to compile fragment shader object.");
				return nullptr;
			}
		}
		...
		if (has_geometry_directive())
		{
			...
			{
				log_error("Failed to build shader program from shader template: Failed to compile geometry shader object.");
				return nullptr;
			}
		}
		...
		if (!program->link())
		{
			log_error("Failed to build shader program from shader template: Failed to link shader program.");
			return nullptr;
		}
	...
	void shader_template::find_directives()
	{
		...
		// Parse directives
		for (usize i = 0; i < m_template_source.lines.size(); ++i)
		{
...
namespace engine::resources
{
	...
	static void handle_includes(std::vector<std::shared_ptr<text_file>>& include_files, text_file& source, std::unordered_set<std::filesystem::path>& include_once, resource_manager& resource_manager)
	{
		...
		for (usize i = 0; i < source.lines.size(); ++i)
		{
			std::string token;
	...
	template <>
	std::unique_ptr<gl::shader_template> resource_loader<gl::shader_template>::load(resources::resource_manager& resource_manager, std::shared_ptr<deserialize_context> ctx)
	{
