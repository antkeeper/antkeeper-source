...
export module engine.gl.pipeline;
export import engine.gl.pipeline_viewport_state;
export import engine.gl.pipeline_rasterization_state;
export import engine.gl.pipeline_depth_stencil_state;
export import engine.gl.pipeline_input_assembly_state;
export import engine.gl.pipeline_vertex_input_state;
export import engine.gl.pipeline_color_blend_state;
export import engine.gl.vertex_array;
export import engine.gl.vertex_buffer;
export import engine.gl.clear_value;
export import engine.gl.framebuffer;
...
export namespace engine::gl
{
	...
	public:
		...
		void bind_vertex_buffers(u32 first_binding, std::span<const vertex_buffer* const> buffers, std::span<const usize> offsets, std::span<const usize> strides);
		...
		void set_viewport(u32 first_viewport, std::span<const viewport> viewports);
		...
		void set_scissor(u32 first_scissor, std::span<const scissor_region> scissors);
		...
		void set_stencil_op(u8 face_mask, stencil_op fail_op, stencil_op pass_op, stencil_op depth_fail_op, gl::compare_op compare_op);
		...
		void set_stencil_compare_mask(u8 face_mask, u32 compare_mask);
		...
		void set_stencil_reference(u8 face_mask, u32 reference);
		...
		void set_stencil_write_mask(u8 face_mask, u32 write_mask);
		...
		void clear_attachments(u8 mask, const clear_value& value);
		...
		[[nodiscard]] inline constexpr const std::array<u32, 2>& get_default_framebuffer_dimensions() const noexcept
		{
			return m_default_framebuffer_dimensions;
		}

		/// Returns the maximum number of supported viewports.
		[[nodiscard]] inline constexpr u32 get_max_viewports() const noexcept
		{
	...
	private:
		...
		void defaut_framebuffer_resized(u32 width, u32 height) noexcept;
		...
		u32 m_max_viewports{1};
		float m_max_sampler_anisotropy{0.0f};
		std::array<u32, 2> m_default_framebuffer_dimensions{0, 0};
