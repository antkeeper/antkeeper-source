...
namespace
{
	...
	void gl_debug_message_callback(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei, const GLchar* message, const void*)
	{
		...
		switch (type)
		{
			...
			case GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR:
			case GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR:
				log_error("OpenGL {} {} ({}) {}: {}", src_str, type_str, severity_str, id, message);
				break;
			
			case GL_DEBUG_TYPE_PORTABILITY:
			case GL_DEBUG_TYPE_PERFORMANCE:
				log_warning("OpenGL {} {} ({}) {}: {}", src_str, type_str, severity_str, id, message);
				break;
			
			case GL_DEBUG_TYPE_MARKER:
			case GL_DEBUG_TYPE_OTHER:
			default:
				log_trace("OpenGL {} {} ({}) {}: {}", src_str, type_str, severity_str, id, message);
				break;
		}
	}
}
...
namespace engine::gl
{
	pipeline::pipeline()
	{
		...
		m_default_framebuffer_dimensions[0] = static_cast<u32>(gl_scissor_box[2]);
		m_default_framebuffer_dimensions[1] = static_cast<u32>(gl_scissor_box[3]);
	...
	void pipeline::bind_vertex_buffers(u32 first_binding, std::span<const vertex_buffer* const> buffers, std::span<const usize> offsets, std::span<const usize> strides)
	{
	...
	void pipeline::set_viewport(u32 first_viewport, std::span<const gl::viewport> viewports)
	{
		// Bounds check
			...
			active_viewport.y != viewport.y)
		{
			...
			(
				...
				math::max(GLsizei{0}, static_cast<GLsizei>(viewport.width)),
				math::max(GLsizei{0}, static_cast<GLsizei>(viewport.height))
			);
		}
	...
	void pipeline::set_scissor(u32 first_scissor, std::span<const gl::scissor_region> scissors)
	{
		// Bounds check
			...
			active_scissor.y != scissor.y)
		{
			...
			(
				...
				math::max(GLsizei{0}, static_cast<GLsizei>(scissor.width)),
				math::max(GLsizei{0}, static_cast<GLsizei>(scissor.height))
			);
		}
	...
	void pipeline::set_stencil_op(u8 face_mask, stencil_op fail_op, stencil_op pass_op, stencil_op depth_fail_op, gl::compare_op compare_op)
	{
	...
	void pipeline::set_stencil_compare_mask(u8 face_mask, u32 compare_mask)
	{
	...
	void pipeline::set_stencil_reference(u8 face_mask, u32 reference)
	{
	...
	void pipeline::set_stencil_write_mask(u8 face_mask, u32 write_mask)
	{
	...
	void pipeline::set_color_write_mask(u8 mask)
	{
	...
	void pipeline::draw(u32 vertex_count, u32 instance_count, u32 first_vertex, u32 first_instance)
	{
	...
	void pipeline::draw_indexed(u32 index_count, u32 instance_count, u32 first_index, i32, u32 first_instance)
	{
		...
		(
			...
			reinterpret_cast<const GLvoid*>(first_index * sizeof(u32)),
			static_cast<GLsizei>(instance_count),
			static_cast<GLuint>(first_instance)
		);
	}

	void pipeline::clear_attachments(u8 mask, const clear_value& value)
	{
	...
	void pipeline::defaut_framebuffer_resized(u32 width, u32 height) noexcept
	{
	...
	void pipeline::fetch_viewport_state()
	{
		...
		{{
			static_cast<i32>(gl_scissor_box[0]),
			static_cast<i32>(gl_scissor_box[1]),
			static_cast<u32>(math::max(0, gl_scissor_box[2])),
			static_cast<u32>(math::max(0, gl_scissor_box[3]))
		}};
	}
	...
	void pipeline::fetch_depth_stencil_state()
	{
		...
		// Stencil front
		{
			...
			m_depth_stencil_state.stencil_front.compare_mask = std::bit_cast<u32>(gl_stencil_front_value_mask);
			m_depth_stencil_state.stencil_front.write_mask = std::bit_cast<u32>(gl_stencil_front_write_mask);
			m_depth_stencil_state.stencil_front.reference = std::bit_cast<u32>(gl_stencil_front_ref);
		}
		...
		// Stencil back
		{
			...
			m_depth_stencil_state.stencil_back.compare_mask = std::bit_cast<u32>(gl_stencil_back_value_mask);
			m_depth_stencil_state.stencil_back.write_mask = std::bit_cast<u32>(gl_stencil_back_write_mask);
			m_depth_stencil_state.stencil_back.reference = std::bit_cast<u32>(gl_stencil_back_ref);
		}
	}
	...
	void pipeline::fetch_color_blend_state()
	{
		...
		m_color_blend_state.color_write_mask =
			static_cast<u8>(gl_color_writemask[0]) |
			(static_cast<u8>(gl_color_writemask[1]) << 1) |
			(static_cast<u8>(gl_color_writemask[2]) << 2) |
			(static_cast<u8>(gl_color_writemask[3]) << 3);
	...
	void pipeline::fetch_clear_value()
	{
		...
		m_clear_value.stencil = static_cast<u32>(gl_stencil_clear);
	}
}
