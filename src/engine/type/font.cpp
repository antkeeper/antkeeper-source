...
namespace engine::type
{
	namespace
	{
		...
		[[nodiscard]] inline constexpr u32 next_power_of_two(u32 n) noexcept
		{
			return u32{1} << std::bit_width(n);
		}
	}
	...
	usize font::cache_glyph(char32_t code)
	{
		return cache_glyphs(code, code);
	}

	usize font::cache_glyphs(char32_t first, char32_t last)
	{
	...
	usize font::cache_glyphs(std::u32string_view text)
	{
		std::vector<glyph*> cached_glyphs;
		bool repack = false;
	...
	usize font::cache_glyphs(std::string_view text)
	{
	...
	math::fvec2 font::get_kerning(char32_t first, char32_t second) const
	{
